...
function test_PlanarFlowLayer_func_size_1(D)
    z = ones(D)
    flow_layer = PlanarFlowLayer(D, logistic)
...
function test_binary_log_likelihood()
    ...
    res = exp(binary_log_likelihood(m, w, X, y))
    (res <= 1) && (res >= 0)
end
...
function test_multi_class_log_likelihood()
    ...
    w = randn(m.structure.n_total_params)
    res = exp(multi_class_log_likelihood(m, w, X, y))
    (res <= 1) && (res >= 0)
end
...
function test_regression_log_likelihood()
    ...
    w = randn(m.structure.n_total_params)
    res = exp(regression_log_likelihood(m, w, X, y))
    (res <= 1) && (res >= 0)
end

function test_log_density()
    m = make_test_VI_model(2, true, 3, log_l = multi_class_log_likelihood)
    X, y = generate_multi_clusters([1,2], [1,2], [3,3])
    log_density(m, randn(m.structure.n_total_params, 5), X, y, coef = 1)
    true
end
...
function test_pred_1()
    m = make_test_VI_model(2, true, 3, log_l = multi_class_log_likelihood)
    X, y = generate_multi_clusters([1,2], [1,2], [3,3])
    res = pred(m, randn(m.structure.n_total_params), X)
    size(res) == size(y)
end
